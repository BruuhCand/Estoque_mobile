...
@Component({
  selector: 'app-proximo-vencimento',
  templateUrl: './proximo-vencimento.page.html',
  styleUrls: ['./proximo-vencimento.page.scss'],
  standalone: true,
  imports: [CommonModule, IonicModule, ListUtilComponent]
})
export class ProximoVencimentoPage implements OnInit {
        ...
        next: (response) => {
          ...
          if(this.produtos.length > 0){
            this.produtos.forEach(x => {
              const validade = new Date(x.validades[0].dataValidade);  
              const hoje = new Date();
              const umaSemana = new Date();
              umaSemana.setDate(hoje.getDate() + 7);
              if(validade < umaSemana && validade >= hoje){
                var y = {
                  id: x.id,
                  nome: x.nome,
